 ...
 *
 * ملاحظة:
 * هذا الملف لا ينفذ صلاحيات المستخدم بنفسه.
 * طبقة المصادقة الحالية في المشروع هي المسؤولة عن
 * تحديد المستخدم المصرح له بالعملية.
 */
    ...
    if (date) {
      sql += `
        ...
        AND (
          ...
          OR s.valid_until >= ?${params.length + 2}
        )
      `;
...
export async function onRequestPost(
  ...
  try {
    ...
    if (
      ...
      /* ---------------------------------------------------
         التأكد أن الموعد من أوقات المعلمة
      --------------------------------------------------- */
        ...
        await db
          .prepare(`
            ...
            WHERE teacher_id = ?1
              ...
              AND requested_start_time = ?3
              AND requested_end_time = ?4
              AND status = 'pending'
...
export async function onRequestPatch(
  ...
  try {
    ...
    if (
      ...
      /* ---------------------------------------------------
         قبول الطلب:
         نتحقق مرة ثانية من عدم وجود تعارض
      --------------------------------------------------- */
