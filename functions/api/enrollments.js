 ...
 *
 * يعتمد فقط على الجداول الموجودة في migrations.
 */
...
async function getEnrollmentCount(db, circleId) {
...
async function getRules(db, circleId, packageId) {
  const result = await db.prepare(`
    ...
    ORDER BY
      ...
      END,
      id DESC
  `).bind(circleId, packageId).all();

  return result.results || [];
}

function ruleValue(rules, type) {
...
function ruleEnabled(rules, type, defaultValue = false) {
  const value = ruleValue(rules, type);
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    /*
     * القواعد مرنة لأن الـSchema يخزن:
     * rule_type + rule_value.
     *
     * القواعد المدعومة هنا:
     * allow_new_students
     * allow_waitlist
      ...
      return json(
        {
          success: false,
          error:
            "New enrollment is disabled by the enrollment rules.",
        },
        409
      );
    }
    ...
    const trialDays =
      configuredTrial !== null
        ? Number(configuredTrial || 0)
...
export async function onRequestGet(context) {
  ...
  if (requestId) {
    ...
    sql += ` AND er.id = ?${params.length}`;
  }

  if (studentId) {
    params.push(Number(studentId));
    sql += ` AND er.student_id = ?${params.length}`;
  }

  if (circleId) {
    params.push(Number(circleId));
    sql += ` AND er.circle_id = ?${params.length}`;
  }
