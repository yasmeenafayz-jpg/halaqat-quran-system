...
function calculateSessionChargeabilityCairo({
   ...
   * لا نستطيع إثبات أن الاعتذار في موعده.
   * لذلك نعامل الغياب كقابل للفوترة.
   */
  if (!sessionStart) {
    return {
      chargeable: true,
      reason:
        "absence_without_valid_session_time"
    };
  }
  ...
  /*
   * قبل انتهاء مهلة الاعتذار:
   * عدم وجود اعتذار حتى الآن لا يعني أنه تأخر.
   */
  if (
    currentTime.getTime() <=
    deadline.getTime()
  ...
  if (
    ...
    return {
      chargeable: true,
      reason:
        "absence_without_valid_excuse_time"
    };
  }
