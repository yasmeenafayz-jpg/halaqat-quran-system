...
async function getReferenceData(db, user) {
  ...
  const paramsTeachers = [];
  const paramsCircles = [];
  const paramsStudents = [];

  let teachersSql = teachersQuery;
  let studentsSql = studentsQuery;

  if (user.role === "teacher") {
    teachersSql += ` AND id = ?`;
    paramsTeachers.push(Number(user.teacher_id));
  } else if (user.role === "student") {
    studentsSql = `
      SELECT id, full_name
      FROM students
      WHERE id = ?
    `;
    paramsStudents.push(Number(user.student_id));
  } else if (user.role === "guardian") {
    studentsSql = `
      ...
      WHERE g.user_id = ?
        ...
        AND (
          s.status IS NULL
          OR s.status NOT IN ('inactive','deleted')
        )
      ORDER BY s.full_name
    `;
    paramsStudents.push(Number(user.id));
  } else if (
    user.role !== "admin" &&
    user.role !== "supervisor"
  ) {
  ...
  const [teachers, circles, students] =
    await Promise.all([
      db
        .prepare(teachersSql + ` ORDER BY full_name`)
        .bind(...paramsTeachers)
        .all(),

      db
        .prepare(circlesQuery)
        .bind(...paramsCircles)
        .all(),
